## Note from Belgrave Henderson
I defined only a few Sass Variables. In a real app more Variables would be defined.
I do not full style the start over page. 
If I had more time I would refactor to not use dangerouslySetInnerHTML.
...
* Prompts appear in the left column, and the essay preview on the right. x
* Each "prompt" consists of a question and an input. x
* When the user blurs the input, the app should "save" their response X and
  update the essay preview. x
...
* Changing one answer should re-roll that answer's template, but not the
  chosen templates of other fields. x
